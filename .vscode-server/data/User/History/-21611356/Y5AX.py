...
def mesh_save_handler(out_file: Types.Mesh, out_name: str) -> None:
    ...
    which can be custom re-configured by the calling function.
    ...
    with _seek_modify(Dirs.CONFIG_FDIR, None, 'Objects') as field:
        field.append(f'{out_name}: {{Id: {len(field)}, Dir: out/{out_name}}}')
        yield field
    # dat = 
    # yield 
    # _seek_modify(Dirs.CONFIG_FDIR, , 'Objects')
    # pm.save_mesh(Dirs.OUT_FDIR, out_file)
